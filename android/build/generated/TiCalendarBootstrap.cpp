...
#define TAG "ti.calendar"

using namespace v8;

static Persistent<Object> bindingCache;

static void TiCalendar_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiCalendar.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TiCalendarBindings::lookupGeneratedInit(
...
static void TiCalendar_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiCalendar_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiCalendar_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TiCalendarBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiCalendarBinding = {
	"ti.calendar",
	TiCalendar_init,
	TiCalendar_dispose
};
...
Java_ti_calendar_TiCalendarBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.calendar", &TiCalendarBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TiCalendarBindings::lookupGeneratedInit));
}
